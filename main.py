    ...
    def move(self, moving_left, moving_right):
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                dx = 0
                # if ai hit wall make them turn around
                if self.char_type == 'enemy':
                    self.direction *=-1
                    self.move_counter = 0
            # check for collision in the y direcion
            if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                # check if below the ground, i.e. jumping
                if self.vel_y < 0:
                    self.vel_y = 0
                    dy = tile[1].bottom - self.rect.top
                # check ifabove the ground, i.e. falling
                elif self.vel_y >= 0:
                    self.vel_y = 0
                    self.in_air = False
                    dy = tile[1].top - self.rect.bottom

        # check if going of the edges
        ...
        if self.char_type == 'player':
            if (self.rect.right > SCREEN_WIDTH - SCROLL_THRESH and bg_scroll < (world.level_lenght * TILE_SIZE) - SCREEN_WIDTH) \
                    or (self.rect.left < SCROLL_THRESH and bg_scroll > dx ):
                self.rect.x -= dx
                screen_scroll = -dx
...
class World():
    ...
    def process_data(self, data):
        ...
        # iterate thorugh each value in level data file
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.right < 0 or self.rect.left > SCREEN_WIDTH:
            self.kill()
        # chack for colliison with walls
        for tile in world.obstacle_list:
